...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # exit(0)
        # print(f'x_pre_view: {x.shape}')
        x = x.view(-1, self.channels, self.size * self.size * self.size)
        # print(f'x_post_view_&_pre_Swap: {x.shape}')
        x = x.swapaxes(1, 2)
        # print(f'x_post_swap: {x.shape}')
        x_ln = self.ln(x)
        # print(f'x_ln: {x_ln.shape}')
        attention_value, _ = self.mha(x_ln, x_ln, x_ln)
        # print(f'attention_value_mha: {attention_value.shape}')
        attention_value = attention_value + x
        # print(f'attention_value_+x: {attention_value.shape}')
        attention_value = self.ff_self(attention_value) + attention_value
        # print(f'attention_value_ff_self: {attention_value.shape}')
...
class Down(nn.Module):
    ...
    # x1_shape: torch.Size([1, 64, 64, 64])
...
class Up(nn.Module):
    ...
    def forward(self, x, skip_x, t):
        # print(f'x: {x.shape}')
        # print(f'add_x_skip: {skip_x.shape}')
        # print(f'pre_up: {x.shape}')
        x = self.up(x)
        # print(f'post_up: {x.shape}')
...
class UNet(nn.Module):
    ...
    def forward(self, x, t):
        ...
        # x2 = self.sa1(x2)
        # print(f'x2_SA_shape: {x2.shape}')
        x3 = self.down2(x2, t)
        # print(f'x3_shape: {x3.shape}')
        # x3 = self.sa2(x3)
        # print(f'x3_SA_shape: {x3.shape}')
        x4 = self.down3(x3, t)
        # print(f'x4_shape: {x4.shape}')
        # x4 = self.sa3(x4)
        # print(f'x4_SA_shape: {x4.shape}')
        # print('DOWN GOOD')
...
if __name__ == '__main__':
    # net = UNet(device="cpu")
    net = UNet_conditional(device="cpu",c_in=1,c_out=1,num_classes=5)
